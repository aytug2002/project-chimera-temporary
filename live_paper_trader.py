...
def execute_trade(decision: dict):
    ...
    order on Alpaca. The logic is now conditional and robust against
    a `None` value for the 'action' key.
    """
    # CRITICAL FIX: Use 'or {}' to ensure 'action' is always a dictionary,
    # even if the agent returns {'action': null}. This prevents the NoneType error.
    ...
    if action_type == 'HOLD':
        print("[Execution] Agent decided to HOLD. No trades will be executed.")
        return None

    print(f"[Execution] Agent action is {action_type}. Clearing existing positions first...")
    try:
    ...
    if action_type in ['BUY', 'SHORT'] and amount_ratio > 0:
        ...
        notional_value = equity * amount_ratio
        notional_value = round(notional_value, 2)
        side = 'buy' if action_type == 'BUY' else 'sell'
        
        print(f"[Execution] Placing new {side} order for notional value: ${notional_value:,.2f}")
        try:
